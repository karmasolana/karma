...
const STORAGE_KEY = "karma-price-history-v2";
// Inception: first LP seed was 0.25 SOL / 0.25 KARMA = 1.0 SOL/KARMA
const INCEPTION_TIME = 1743284400000; // approx March 29 2026 21:00 UTC
...
export default function PriceChart({ karmaPrice, solPrice }: { karmaPrice: number; solPrice: number | null }) {
  ...
  // Load history + add current point
  useEffect(() => {
    const h = loadHistory();
    const now = Date.now();
    const last = h[h.length - 1];
    if (!last || now - last.time > 30000) { // add point every 30s
      h.push({ time: now, price: karmaPrice });
      saveHistory(h);
    }
    setHistory(h);
  }, [karmaPrice]);
  ...
  useEffect(() => {
    ...
    ctx.fillText(mode === "USD" ? `$${last.toFixed(2)}` : `${last.toFixed(4)} SOL`, w - 4, 12);
    ctx.fillStyle = "#444"; ctx.textAlign = "left";
    const first = prices[0];
    ctx.fillText(mode === "USD" ? `$${first.toFixed(2)}` : `${first.toFixed(4)}`, 4, 12);
